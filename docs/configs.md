<!-- This file is auto-generated by ConfigGen from BunnyGardenFixMod/Configs.yaml. Do not edit manually. -->

# Config 設定一覧

ゲームを一度起動すると `BepInEx/config/net.noeleve.BunnyGardenFixMod.cfg` が生成されます。
...
| `ToggleOverlayKey` | `F12` | オーバーレイ表示切替のキーボードキー<br>フリーカメラ・時間停止の操作ガイド表示が対象です。 |
| `ToggleOverlayButton` | `Start` | オーバーレイ表示切替のボタン<br>フリーカメラ・時間停止の操作ガイド表示が対象です。<br>ControllerModifier と同時押しが必要です。 |
